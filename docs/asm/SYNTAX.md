...
- `.bss` declares named global slots, zero-initialized at startup. Must appear before `.data` or `.main`.
...
MOV R1, [counter]     ; load counter into R1
MOV R1, counter       ; load slot index of counter into R1 (address-of)
```
...
```asm
MOV R1, 67          ; register - immediate (i32)
MOV R1, R2         ; register - register
MOV R1, [name]      ; register - bss slot value
MOV [name], R1      ; bss slot - register
MOV [name], 67       ; bss slot - immediate
MOV R1, name        ; register - bss slot index (address-of)
...
- 99 general-purpose registers: `R0` through `R98`.
- Register names are case-insensitive (`R1` and `R1` are equivalent).
- Single-digit registers must be zero-padded: use `R1`, not `R1`.
- `R0` is the conventional return value register for function calls.
