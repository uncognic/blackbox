...
void write_u64(FILE* out, uint64_t val) {
    const std::array<uint8_t, 8> bytes = {
        static_cast<uint8_t>(val >> 0),  static_cast<uint8_t>(val >> 8),
        static_cast<uint8_t>(val >> 16), static_cast<uint8_t>(val >> 24),
        static_cast<uint8_t>(val >> 32), static_cast<uint8_t>(val >> 40),
        static_cast<uint8_t>(val >> 48), static_cast<uint8_t>(val >> 56),
    };
    write_bytes(out, bytes);
}
...
bool read_u32_le(const std::vector<uint8_t>& bytes, size_t index, uint32_t& out) {
    ...
    out = static_cast<uint32_t>(bytes[index]) | (static_cast<uint32_t>(bytes[index + 1]) << 8U) |
...
bool read_u64_le(const std::vector<uint8_t>& bytes, size_t index, uint64_t& out) {
    ...
    out = static_cast<uint64_t>(bytes[index]) | (static_cast<uint64_t>(bytes[index + 1]) << 8U) |
