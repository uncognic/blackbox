...
#define RAISE_FAULT(type, msg, ...)                             \
    ...
    while (pc < size)
    {
        ...
        case OPCODE_SYSCALL:
        {
            ...
            {
                blackbox::fmt::err_fmt( "FAULT: SYSCALL only allowed in protected mode at pc=%zu\n", pc);
                goto fault_exit;
            }
        ...
        case OPCODE_FAULTRET:
            REQUIRE_PRIVILEGED("FAULTRET")
            {
                ...
                {
                    blackbox::fmt::err_fmt( "FAULTRET executed but no fault is active at pc=%zu\n", pc);
                    goto fault_exit;
                }
        ...
        case OPCODE_GETENV:
        {
            ...
            if (pc + envlen > size)
            {
                blackbox::fmt::err_fmt( "Environment variable name past end of program at pc=%zu\n", pc);
                goto fault_exit;
            }
            char envname[256];
