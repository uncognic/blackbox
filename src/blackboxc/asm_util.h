...
bool build_macro_owned(const std::string &name, const std::vector<std::string> &params, const std::vector<std::string> &body, Macro &out);
int collect_lines_from_buffer(const char *src, std::vector<std::string> &lines);

template <typename... Args>
void dbg(bool debug, const char *fmt, const Args &...args)
{
    if (debug)
        printf(fmt, args...);
}
